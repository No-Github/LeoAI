...
- 首次登录新增独立强制改密页面；登录、刷新和受限接口响应均可识别改密状态并阻止提前进入工作台
- 前端入口采用禁止缓存策略，哈希资源使用长期不可变缓存；版本切换遇到旧 Chunk 时自动刷新恢复
...
- JDK 17 执行完整 Maven Reactor 测试：共 409 项，388 项通过，21 项因本机 PHP CLI、扩展或外部环境条件不足跳过，0 失败
