...
public class HttpProxyHandleClientThread implements Runnable {
    ...
    public HttpProxyHandleClientThread(Socket clientSocket, ComponentInvokeCapable puppetNode,
    ...
    private void handlePlainHttp(String firstLine, String header, byte[] headerBytes,
        ...
        Map<String, Object> writeRes = puppetNode.invokeComponent("ProxyForwardComponent", params);
        if (!Integer.valueOf(200).equals(writeRes.get("code"))) {
            ...
            try { puppetNode.invokeComponent("ProxyForwardComponent", closeParams); } catch (Exception ignored) {}
            close(clientSocket);
            return;
        }
    ...
    private void recordAndRelay() throws InterruptedException {
        ...
        try {
            Thread r = new Thread(new ReadDataThread(clientSocket, connId, puppetNode, statistics));
            Thread w = new Thread(new WriteDataThread(clientSocket, connId, puppetNode, statistics));
            r.setDaemon(true);
            w.setDaemon(true);
            r.start(); w.start();
            r.join(); w.join();
