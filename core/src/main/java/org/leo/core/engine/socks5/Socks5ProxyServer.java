...
public class Socks5ProxyServer implements Runnable {
    ...
    public Socks5ProxyServer(ComponentInvokeCapable puppetNode, int listenPort) {
    ...
    public void run() {
        while (running) {
            try {
                ...
                Thread thread=new Thread(new HandleClientThread(clientSocket, puppetNode, statistics));
                thread.start();
