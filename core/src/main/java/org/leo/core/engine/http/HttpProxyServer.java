...
public class HttpProxyServer implements Runnable {
    ...
    public HttpProxyServer(ComponentInvokeCapable puppetNode, int listenPort) {
    ...
    public void run() {
        while (running) {
            try {
                ...
                Thread t = new Thread(new HttpProxyHandleClientThread(client, puppetNode, statistics));
                t.setDaemon(true);
                t.start();
            } catch (Exception e) {
