...
public class HandleClientThread implements Runnable{
    ...
    private ComponentInvokeCapable puppetNode;
    private Socks5ProxyStatistics statistics;
    private String connId;
    private String targetHost;
    private int targetPort;

    public HandleClientThread(Socket clientSocket, ComponentInvokeCapable puppetNode, Socks5ProxyStatistics statistics) {
    ...
    public void run() {
        ...
        if (parseSocks5(outputStream,inputStream)){
            ...
            try {
                Thread readDataThread=new Thread(new ReadDataThread(clientSocket, connId, puppetNode, statistics));
                Thread writeDataThread=new Thread(new WriteDataThread(clientSocket, connId, puppetNode, statistics));
